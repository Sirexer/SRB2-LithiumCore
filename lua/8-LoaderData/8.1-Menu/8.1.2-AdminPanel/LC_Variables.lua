...
local t = {
	name = "LC_MENU_SERVERVARIABLES",
	type = "admin",
	description = "LC_MENU_SERVERVARIABLES_TIP",
	funchud = function(v)
		...
		-- Arrow Down
		if LC_menu.nav+1 != LC_menu.lastnav and #LC.menu.vars > maxslots and first+maxslots-1 < #LC.menu.vars
			v.drawString(318, 180+arrow_y, "\x82"..string.char(27), V_SNAPTOBOTTOM, "right")
		end
		local y = 0
		for i = first, first+maxslots-1 do
			if LC.menu.vars and LC.menu.vars[i] and LC.menu.vars[i].command
	...
	funcenter = function()
		...
		local LC_menu = LC.menu.player_state
		LC_menu.nav = 0
		LC_menu.lastnav = #LC.menu.vars-1
	end,
	...
	funchook = function(key)
		...
		if LC.functions.GetMenuAction(key.name) == LCMA_NAVLEFT
			CV_AddValue(LC.menu.vars[LC_menu.nav+1].command, -step)
